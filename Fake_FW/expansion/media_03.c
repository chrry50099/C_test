...
#define MEDIA_03_WATERMARK (68u)

void fw_media_03_init(fw_media_03_state_t *state)
{
    state->counter = 3u;
    state->watermark = MEDIA_03_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_media_03_tick(fw_media_03_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_media_04_state_t next_state;
        fw_media_04_init(&next_state);
        state->watermark += fw_media_04_score(&next_state) & 0x3u;
    }
