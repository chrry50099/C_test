...
#define QOS_03_WATERMARK (68u)

void fw_qos_03_init(fw_qos_03_state_t *state)
{
    state->counter = 3u;
    state->watermark = QOS_03_WATERMARK;
    state->last_event = 0u;
    state->last_status = FW_OK;
}

fw_status_t fw_qos_03_tick(fw_qos_03_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_qos_04_state_t next_state;
        fw_qos_04_init(&next_state);
        state->watermark += fw_qos_04_score(&next_state) & 0x3u;
    }
...
uint32_t fw_qos_03_score(const fw_qos_03_state_t *state)
{
