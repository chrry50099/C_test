...
#define ECC_04_WATERMARK (85u)

void fw_ecc_04_init(fw_ecc_04_state_t *state)
{
    state->counter = 4u;
    state->watermark = ECC_04_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_ecc_04_tick(fw_ecc_04_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_ecc_05_state_t next_state;
        fw_ecc_05_init(&next_state);
        state->watermark += fw_ecc_05_score(&next_state) & 0x3u;
    }
