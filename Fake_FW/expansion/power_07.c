...
#define POWER_07_WATERMARK (136u)

void fw_power_07_init(fw_power_07_state_t *state)
{
    state->counter = 7u;
    state->watermark = POWER_07_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_power_07_tick(fw_power_07_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_power_08_state_t next_state;
        fw_power_08_init(&next_state);
        state->watermark += fw_power_08_score(&next_state) & 0x3u;
    }
