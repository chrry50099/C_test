...
#define DMA_08_WATERMARK (153u)

void fw_dma_08_init(fw_dma_08_state_t *state)
{
    state->counter = 8u;
    state->watermark = DMA_08_WATERMARK;
    state->last_event = 0u;
    state->last_status = FW_OK;
}

fw_status_t fw_dma_08_tick(fw_dma_08_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_dma_09_state_t next_state;
        fw_dma_09_init(&next_state);
        state->watermark += fw_dma_09_score(&next_state) & 0x3u;
    }
...
uint32_t fw_dma_08_score(const fw_dma_08_state_t *state)
{
