...
#define TELEMETRY_08_WATERMARK (153u)

void fw_telemetry_08_init(fw_telemetry_08_state_t *state)
{
    state->counter = 8u;
    state->watermark = TELEMETRY_08_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_telemetry_08_tick(fw_telemetry_08_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_telemetry_09_state_t next_state;
        fw_telemetry_09_init(&next_state);
        state->watermark += fw_telemetry_09_score(&next_state) & 0x3u;
    }
