...
#include "expansion/media_06.h"

#define MEDIA_05_WATERMARK (102u)

void fw_media_05_init(fw_media_05_state_t *state)
{
    state->counter = 5u;
    state->watermark = MEDIA_05_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_media_05_tick(fw_media_05_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_media_06_state_t next_state;
        fw_media_06_init(&next_state);
        state->watermark += fw_media_06_score(&next_state) & 0x3u;
    }
