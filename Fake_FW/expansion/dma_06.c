...
#define DMA_06_WATERMARK (119u)

void fw_dma_06_init(fw_dma_06_state_t *state)
{
    state->counter = 6u;
    state->watermark = DMA_06_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_dma_06_tick(fw_dma_06_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_dma_07_state_t next_state;
        fw_dma_07_init(&next_state);
        state->watermark += fw_dma_07_score(&next_state) & 0x3u;
    }
