...
#define THERMAL_08_WATERMARK (153u)

void fw_thermal_08_init(fw_thermal_08_state_t *state)
{
    state->counter = 8u;
    state->watermark = THERMAL_08_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_thermal_08_tick(fw_thermal_08_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_thermal_09_state_t next_state;
        fw_thermal_09_init(&next_state);
        state->watermark += fw_thermal_09_score(&next_state) & 0x3u;
    }
