...
#include "expansion/media_09.h"

#define MEDIA_08_WATERMARK (153u)

void fw_media_08_init(fw_media_08_state_t *state)
{
    state->counter = 8u;
    state->watermark = MEDIA_08_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_media_08_tick(fw_media_08_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_media_09_state_t next_state;
        fw_media_09_init(&next_state);
        state->watermark += fw_media_09_score(&next_state) & 0x3u;
    }
