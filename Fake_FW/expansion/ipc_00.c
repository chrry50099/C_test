...
#define IPC_00_WATERMARK (17u)

void fw_ipc_00_init(fw_ipc_00_state_t *state)
{
    state->counter = 0u;
    state->watermark = IPC_00_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_ipc_00_tick(fw_ipc_00_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_ipc_01_state_t next_state;
        fw_ipc_01_init(&next_state);
        state->watermark += fw_ipc_01_score(&next_state) & 0x3u;
    }
