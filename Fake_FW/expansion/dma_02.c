...
#define DMA_02_WATERMARK (51u)

void fw_dma_02_init(fw_dma_02_state_t *state)
{
    state->counter = 2u;
    state->watermark = DMA_02_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_dma_02_tick(fw_dma_02_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_dma_03_state_t next_state;
        fw_dma_03_init(&next_state);
        state->watermark += fw_dma_03_score(&next_state) & 0x3u;
    }
