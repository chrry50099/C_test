#include "expansion/qos_01.h"
#include "expansion/qos_02.h"

#define QOS_01_WATERMARK (34u)

void fw_qos_01_init(fw_qos_01_state_t *state)
{
    state->counter = 1u;
    state->watermark = QOS_01_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_qos_01_tick(fw_qos_01_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_qos_02_state_t next_state;
        fw_qos_02_init(&next_state);
        state->watermark += fw_qos_02_score(&next_state) & 0x3u;
    }
