...
#define QOS_00_WATERMARK (17u)

void fw_qos_00_init(fw_qos_00_state_t *state)
{
    state->counter = 0u;
    state->watermark = QOS_00_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_qos_00_tick(fw_qos_00_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_qos_01_state_t next_state;
        fw_qos_01_init(&next_state);
        state->watermark += fw_qos_01_score(&next_state) & 0x3u;
    }
