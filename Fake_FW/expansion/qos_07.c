...
#define QOS_07_WATERMARK (136u)

void fw_qos_07_init(fw_qos_07_state_t *state)
{
    state->counter = 7u;
    state->watermark = QOS_07_WATERMARK;
    state->last_event = 0u;
    state->last_status = FW_OK;
}

fw_status_t fw_qos_07_tick(fw_qos_07_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_qos_08_state_t next_state;
        fw_qos_08_init(&next_state);
        state->watermark += fw_qos_08_score(&next_state) & 0x3u;
    }
...
uint32_t fw_qos_07_score(const fw_qos_07_state_t *state)
{
