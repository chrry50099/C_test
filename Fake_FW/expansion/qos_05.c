...
#define QOS_05_WATERMARK (102u)

void fw_qos_05_init(fw_qos_05_state_t *state)
{
    state->counter = 5u;
    state->watermark = QOS_05_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_qos_05_tick(fw_qos_05_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_qos_06_state_t next_state;
        fw_qos_06_init(&next_state);
        state->watermark += fw_qos_06_score(&next_state) & 0x3u;
    }
