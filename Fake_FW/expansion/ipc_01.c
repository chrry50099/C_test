#include "expansion/ipc_01.h"
#include "expansion/ipc_02.h"

#define IPC_01_WATERMARK (34u)

void fw_ipc_01_init(fw_ipc_01_state_t *state)
{
    state->counter = 1u;
    state->watermark = IPC_01_WATERMARK;
    state->last_event = 0u;
    state->last_status = FW_OK;
}

fw_status_t fw_ipc_01_tick(fw_ipc_01_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_ipc_02_state_t next_state;
        fw_ipc_02_init(&next_state);
        state->watermark += fw_ipc_02_score(&next_state) & 0x3u;
    }
...
uint32_t fw_ipc_01_score(const fw_ipc_01_state_t *state)
{
