...
#define MEDIA_01_WATERMARK (34u)

void fw_media_01_init(fw_media_01_state_t *state)
{
    state->counter = 1u;
    state->watermark = MEDIA_01_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_media_01_tick(fw_media_01_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_media_02_state_t next_state;
        fw_media_02_init(&next_state);
        state->watermark += fw_media_02_score(&next_state) & 0x3u;
    }
