...
#define IPC_06_WATERMARK (119u)

void fw_ipc_06_init(fw_ipc_06_state_t *state)
{
    state->counter = 6u;
    state->watermark = IPC_06_WATERMARK;
    state->last_event = 0u;
...
fw_status_t fw_ipc_06_tick(fw_ipc_06_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_ipc_07_state_t next_state;
        fw_ipc_07_init(&next_state);
        state->watermark += fw_ipc_07_score(&next_state) & 0x3u;
    }
