...
#define DMA_00_WATERMARK (17u)

void fw_dma_00_init(fw_dma_00_state_t *state)
{
    state->counter = 0u;
    state->watermark = DMA_00_WATERMARK;
    state->last_event = 0u;
    state->last_status = FW_OK;
}

fw_status_t fw_dma_00_tick(fw_dma_00_state_t *state, uint32_t event_id)
{
    ...
    if ((event_id & 1u) == 0u) {
        fw_dma_01_state_t next_state;
        fw_dma_01_init(&next_state);
        state->watermark += fw_dma_01_score(&next_state) & 0x3u;
    }
...
uint32_t fw_dma_00_score(const fw_dma_00_state_t *state)
{
